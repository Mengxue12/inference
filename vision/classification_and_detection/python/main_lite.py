...
def get_args():
    ...
    parser.add_argument("--use_cache", type=int, default=0, help="use cache")
    parser.add_argument(
    ...
    def run_one_item(self, qitem):
        ...
        try:
            ...
            if self.debug_labels:
                scores = np.asarray(results[0])
                for idx, content_id in enumerate(qitem.content_id):
                    top_k = np.argsort(scores[idx])[-5:][::-1]
                    label_offset = getattr(self.post_process, "offset", 0)
                    predicted = int(top_k[0]) + label_offset
                    expected = int(qitem.label[idx])
                    log.info(
                        "label debug: content_id=%d image=%s expected=%d "
                        "predicted=%d top5=%s",
                        content_id,
                        self.ds.get_item_loc(content_id),
                        expected,
                        predicted,
                        [
                            (int(class_id) + label_offset, float(scores[idx][class_id]))
...
def main():
    ...
    if args.backend == "tflite":
        pre_proc = dataset.pre_process_imagenet
    if 'efficientnet' in str(args.model) or 'efficientnet' in str(args.model_name):
        pre_proc = dataset.pre_process_imagenet_efficientnet
    if 'int8' in str(args.model) or 'int8' in str(args.model_name):
        pre_proc = dataset.pre_process_imagenet_int8
    if args.use_preprocessed_dataset:
        pre_proc = None
    ds = wanted_dataset(
        data_path=args.dataset_path,
        image_list=args.dataset_list,
        name=str(kwargs["image_size"][0])+'_int8' if 'int8' in str(args.model) else str(kwargs["image_size"][0]),
